...
class UserPassiveBehavior(AbstractBehavior):
    @staticmethod
    async def stack_processing(entity, data):
        data.data_others["path"].append(entity.position_3D)
        link_state = _get_link_state(entity=entity, cross_layer_message=data)
        if link_state is not None:
            if not link_state.is_available:
                NetworkPerformance.packet_loss(
                    data_size_byte=data.data_others["data_size_byte"],
                    reason="physical layer " + link_state.reason,
                )
                return
            data.data_others["delay"] += link_state.total_link_delay_ms
            data.data_others["last_link_physical_state"] = PhysicalLayerModel.state_to_dict(link_state)

        cross_layer_message = StackFunc.stack_processing(entity=entity, cross_layer_message=data)
        if cross_layer_message:
            interface = "Default"
            to_buffer = entity.mac_table['*'][interface]
            await to_buffer.put(cross_layer_message)
        return
...
def _get_link_state(entity, cross_layer_message) -> LinkPhysicalState or None:
    ...
    source_ip = ip_list[-1] if ip_list is not None and len(ip_list) >= 1 else "unknown"
